...
> 基于 Linear 设计语言，为 Web 日历排程工具定制的设计系统。
> 亮色模式默认，同时覆盖暗色模式。设计核心理念：极致精确、信息密集但不拥挤、色彩仅用于传达信息而非装饰。

---

## 1. Visual Theme & Atmosphere

CleanCalendar 是一款 Web 效率工具。它的视觉语言应当传达**精确、克制、可靠**——不是花哨的 SaaS 落地页，而是一个你每天打开、安静服务于你的工具。

**亮色模式（默认）：** 暖白基底（`#fafaf9`）上，信息通过灰度的精确层级浮现。日程网格是整个界面的视觉重心，任务格子是仅有的色彩载体——标签组颜色在亮色背景上降低饱和度，保持可读性的同时不失辨识度。边框极细、半透，干净利落。

**暗色模式：** 转译为近黑画布（`#0d0d0e`），保持同样的克制——白色文本层级、半透边框、任务格子用标签组颜色但调亮以适应暗色背景。

**关键特征：**
- 亮色原生设计（默认）：`#fafaf9` 基础画布，`#f5f4f3` 面板，`#ffffff` 浮层
- 暗色模式完整覆盖：`#0d0d0e` / `#121213` / `#1a1a1c`
...
- 亮色模式（默认）：任务格子背景 = 标签组颜色 + 透明度 `0.15`，文字 = 标签组颜色（适当调暗）
- 暗色模式：任务格子背景 = 标签组颜色 + 透明度 `0.18`，文字 = 标签组颜色（适当调亮）

> **设计原则：** 系统 UI（按钮、输入框、面板）全部中性灰度，不带蓝色/紫色调。仅两类元素使用色彩：(1) 用户标签组颜色——任务格子和侧栏色点；(2) 语义色——绿色表示正向操作和开启状态，红色表示危险操作，五圆评分按 1-5 从绿过渡到红。`--accent`、`--switch-on`、`--status-*` 在亮色/暗色模式下保持不变，不随主题变化。
...
| `--btn-ghost-bg` | `rgba(255,255,255,0.06)` | Ghost 按钮背景 |
...
> `--switch-on`、`--status-success`、`--status-warning`、`--status-error` 在暗色模式下保持不变，与亮色模式一致。
...
**开关 (Toggle):**
```
- 外框: 30×18px pill (--radius-full), 默认 --border-default 背景, box-sizing: border-box
- 开启: --switch-on (#27a644) 绿色背景
- 滑块: 14px 白色圆形 (#ffffff), 始终白色（不随亮暗模式变化）, top: 1px; left: 1px
- 滑块位移: translateX(13px)
- 过渡: 150ms
- 用途: "可碎片化处理"、"可影响其他任务"
```
...
亮色模式下使用阴影表达深度，暗色模式下使用**亮度步进** + **半透边框**：

| Level | Treatment (Light) | Treatment (Dark) | Usage |
|-------|-------------------|------------------|-------|
| Canvas | `--bg-canvas` | `--bg-canvas` | 日程表背景 |
| Panel | `--bg-panel` + `--border-default` | `--bg-panel` + `--border-default` | 侧栏、顶栏、详情面板 |
| Surface | `--bg-surface` + `box-shadow: 0 2px 8px rgba(0,0,0,0.08)` | `--bg-surface` + `--border-default` | 弹窗、下拉菜单 |
| Overlay | `rgba(0,0,0,0.15)` | `rgba(0,0,0,0.6)` | 模态框遮罩 |

暗色模式下 Elevation 不通过阴影叠加，而是**每升一级，背景白色不透明度 + 0.02~0.04**。亮色模式下反弹——每升一级背景变亮，再加细微阴影。
...
- ❌ 不要在亮色背景上用纯黑 `#000000` 做主文本——`#1a1a1c` 更柔和
...
CleanCalendar 是 Web 应用，建议窗口最小宽度 900px。同时提供基础移动端适配：

- 窗口宽度 ≥ 1200px：完整三栏布局（侧栏 240 + 日程表 + 详情面板 280）
- 窗口宽度 900-1200px：详情面板自动折叠
- 窗口宽度 680-900px：侧栏和详情面板均折叠
- 窗口宽度 < 680px：单栏移动端布局，日程表竖向堆叠
- 任务格子文字在格子宽度 < 80px 时仅显示 emoji
- 时间粒度较低时（60min），格子的最小高度增大以留出文字空间
- 弹窗组件（下拉菜单、日历选择器）在小屏幕上自适应为居中 bottom sheet 风格
...
### Quick Reference
- Canvas: `#fafaf9` (light, default) / `#0d0d0e` (dark)
- Panel: `#f5f4f3` (light) / `#121213` (dark)
- Primary text: `#1a1a1c` (light) / `#f0f0f2` (dark)
- Secondary text: `#585a60` (light) / `#c8ccd4` (dark)
- Accent (neutral): `#6b6e75`
- Success (green): `#27a644`
- Danger (red): `#e5484d`
- Border: `rgba(0,0,0,0.08)` (light) / `rgba(255,255,255,0.06)` (dark)
- Task block: tag color + 0.15 opacity bg (light) / 0.18 opacity (dark)
- Grid header: `#f3f2f0` (light) / `#0f0f11` (dark)
- Today highlight: `rgba(0,0,0,0.03)` (light) / `rgba(255,255,255,0.03)` (dark)
- Font: Inter Variable, `"cv01" "ss03"`, weight 510 emphasis

### Component Snapshot
```
Task Block = tagColor @ 0.15 opacity bg + tagColor text + 6px radius + 13px Inter 400
  └── ×0.90 brightness on text for light mode (×1.15 for dark mode)
...
Switch = 30×18px pill + box-sizing:border-box + green on + white (#fff) knob always | translateX(13px)
Custom Select Popup = 8px radius + --bg-surface + --border-strong + box-shadow (light) / border (dark)
Grid Line = rgba(0,0,0,0.05) 1px (light) / rgba(255,255,255,0.04) 1px (dark)
```
